...
export const getDanhSachPhong = createAsyncThunk(
    ...
    async (idViTri, thunkAPI) => {
        try {
            ...
            let dataUpdate = null;
            if (
                thunkAPI.getState().bookingRoomSlice.bookingLocation
                    .idLocation ===
                ...
                dataUpdate = result.data.filter(
                    (item) =>
                        item.guests >=
                        thunkAPI.getState().bookingRoomSlice.totalCustomer
                );
            } else {
...
const danhSachPhongSlice = createSlice({
    ...
    reducers: {
        ...
        handleCheckbox: (state, { payload }) => {
            let index = state.valueCheckbox.findIndex(
                (item) => item === payload
            );
            if (index !== -1) {
            ...
            let dataCheck = state.valueCheckbox
                ?.map((item) => {
                    let arrUpdate = arrConvenient.filter(
                        (check) => check.id === item
                    );
                    return arrUpdate;
                })
        ...
        handleSearchRoomList: (state) => {
            ...
            if (
                ...
                arrData2 = handleDataSaveArrButton(
                    arrData1,
                    state.dataValueButton
                );
                arrData3 = handleDataSaveArrCheckbox(
                    arrData2,
                    state.dataCheckbox
                );
                ...
                arrData3 = handleDataSaveArrButton(
                    arrData1,
                    state.dataValueButton
                );
            } else {

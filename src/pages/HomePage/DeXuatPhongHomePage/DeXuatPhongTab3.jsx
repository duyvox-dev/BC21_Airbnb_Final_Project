...
export default function DeXuatPhongTab3(props) {

    let navigate = useNavigate();

    let { deXuatDanhSachPhongTab3 } = props;
    ...
    const SliderSettings = {
        dots: false,
        infinite: deXuatDanhSachPhongTab3 > 4 ? true : false,
        speed: 500,
        slidesToShow: 4,
        slidesToScroll: 1,
    };

    const renderDeXuatPhongTab3 = () => {
        return deXuatDanhSachPhongTab3.map((phong, index) => {
    ...
    const handleClick = () => {
        navigate(`/search/${_.first(deXuatDanhSachPhongTab3).locationId.province}`)
    };

    return (
        <div className="relative">
            {
                deXuatDanhSachPhongTab3.length > 0 && <Fragment>
                    <Slider
                        ref={customSlider}
                        {...SliderSettings}
                        className='w-full'
                    >
                        {renderDeXuatPhongTab3()}
                    </Slider>
                    <button

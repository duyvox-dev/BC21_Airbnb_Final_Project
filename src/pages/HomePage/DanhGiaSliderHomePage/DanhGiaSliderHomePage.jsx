...
export default function DanhGiaSliderHomePage(props) {
    ...
    useEffect(() => {
        //Set lại giá trị currentScreenDimesion mỗi khi resize kích cỡ màn hình
        if (checkScreenDimension.width > 1396) {
            setCurrentScreenDimesion(3);
        }
        if (
            checkScreenDimension.width > 768 &&
            checkScreenDimension.width <= 1396
        ) {
            setCurrentScreenDimesion(2);
        }
        if (
            checkScreenDimension.width > 640 &&
            checkScreenDimension.width <= 768
        ) {
            setCurrentScreenDimesion(2);
        }
        if (
            checkScreenDimension.width > 320 &&
            checkScreenDimension.width <= 640
        ) {
            ...
            return (
                ...
                >
                    <div className="w-80 h-80 mx-auto p-2 border border-neutral-400 border-b-4 border-b-rose-600 relative group-hover:bg-neutral-200 group-hover:scale-95">
                        <h2 className="text-center text-xl my-auto text-rose-500">
                            {danhGia.roomId?.name}
                        </h2>
                        <h3 className="text-lg text-center my-auto mt-7">
                            {danhGia.content}
                        </h3>
                        {danhGia.userId?.name !== undefined ||
                        danhGia.userId?.address !== undefined ? (
                            <div className="absolute bottom-2 w-full flex justify-center">
                                <p className="text-base text-neutral-500 italic text-center my-auto">
                                    {danhGia.userId?.name} từ{" "}
                                    {danhGia.userId?.address}
                                </p>
                            </div>
                        ) : (
                            <Fragment />
